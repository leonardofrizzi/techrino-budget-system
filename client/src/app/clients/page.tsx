  ...
  useEffect(() => {
    async function loadClients() {
      ...
      try {
        ...
        setClients((data as Client[]).map((c: Client) => ({ ...c, status: c.status ?? 'active' })) );
      } catch {
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center"><Plus size={16} className="mr-2"/> New Client</button>
      </div>
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="flex items-center bg-gray-100 rounded px-3 flex-1">
          <Search size={18} className="text-gray-500"/>
          <input type="text" placeholder="Search clients..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="p-2.5 bg-transparent w-full outline-none"/>
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as 'all'|'active'|'inactive')} className="p-2.5 border border-gray-300 rounded min-w-[180px]"><option value="all">All clients</option><option value="active">Active clients</option><option value="inactive">Inactive clients</option></select>
      </div>
      {filtered.length === 0 ? (
              ...
              {filtered.map(c => (
                <tr key={c.id} className="hover:bg-blue-50">
                  ...
                  <td className="py-3 px-4 border-b text-center">
                    <button onClick={() => toggleStatus(c)} className={`px-4 py-1 min-w-[6rem] rounded-full text-xs font-medium transition-colors ${c.status==='active'?'bg-green-100 text-green-800 hover:bg-green-200':'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}>{(c.status ?? 'active').charAt(0).toUpperCase() + (c.status ?? 'active').slice(1)}</button>
                  </td>
                  <td className="py-3 px-4 border-b text-center">
                    <div className="flex justify-center gap-4">
                      <button onClick={() => openEdit(c)} title="Edit" className="p-1 rounded hover:bg-gray-100 text-gray-600 hover:text-blue-600"><Edit size={18}/></button>
                      <button onClick={() => deleteClient(c.id)} title="Delete" className="p-1 rounded hover:bg-gray-100 text-gray-600 hover:text-red-600"><Trash2 size={18}/></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-3">
              ...
              <div className="flex justify-end space-x-2">
                ...
                <button type="submit" disabled={creating} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center">{creating?<Loader2 size={16} className="animate-spin mr-2"/>:null}Save</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-3">
              ...
              <div className="flex justify-end space-x-2">
                <button type="button" onClick={closeEdit} className="px-4 py-2 border rounded hover:bg-gray-100">Cancel</button>
                <button type="submit" disabled={editing} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center">{editing?<Loader2 size={16} className="animate-spin mr-2"/>:null}Save</button>
              </div>
            </form>
          </div>
        </div>
